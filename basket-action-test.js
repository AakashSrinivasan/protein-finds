...
async function run(engine,offline){
  ...
  await page.click('[data-tab="basket"]');assert.equal(await page.locator('[data-screen="basket"] [data-remove="magic-spoon"]').count(),1);assert.match(await page.locator('[data-screen="basket"]').innerText(),/High-protein cereal/);
  ...
  await page.click('[data-tab="screener"]');await page.click('[data-screen-reset]');await page.selectOption('#criterionPicker','numeric:protein');await page.click('[data-add-criterion]');
  ...
  assert.deepEqual(errors,[]);await browser.close();console.log(`PASS basket contract ${engine.name()} ${offline?'offline':'online'}`);
}

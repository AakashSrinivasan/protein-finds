...
function extractLegacyRelease(destination) {
  ...
  assert.equal(result.status, 0, `legacy release archive failed: ${result.stderr}`);
...
(async () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      ['discover', 'screener', 'nearby', 'ask', 'basket'],
    ...
    for (const asset of ['app-shell.css', 'data.js', 'product-screener.js', 'location-data.js', 'ask-protein.js', 'app.js']) {
      assert.ok(criticalAssets.some(url => url.includes(`/${asset}?v=12`)), `${asset} loads through its v12 cache-miss URL`);
    }

    await returningPage.locator('[data-tab="screener"]').click();
    await returningPage.waitForSelector('[data-screen="screener"]');
    await returningPage.locator('#screenBuilder > summary').click();
    await returningPage.fill('#screenMinProtein', '10');
    await returningPage.locator('#screenMinProtein').press('Tab');
    ...
    await returningPage.evaluate(() => document.querySelector('[data-screen-result] .screen-product a').click());
    await returningPage.waitForSelector('[data-screen="product"]');
    const savedScroll = await returningPage.evaluate(() => JSON.parse(sessionStorage.getItem('protein-finds-scroll-v1') || '{}').screener);
    assert.ok(savedScroll > 0, 'Screener scroll position is persisted before product navigation');
    ...
    assert.equal(await returningPage.inputValue('#screenMinProtein'), '10', 'Screener state survives the first upgraded visit');

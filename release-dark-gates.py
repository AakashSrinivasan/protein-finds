...
try:
    if local:
        for attempt in range(30):
            try:
                served = urllib.request.urlopen(url.replace('index.html', 'dark-premium.css')).read()
                break
            except OSError:
                time.sleep(.2)
        assert served == (root / 'dark-premium.css').read_bytes()
    out = root / 'review/dark-premium'
    ...
    subprocess.run(['node', 'release-dark-capture.js'], env=dict(os.environ, REVIEW_URL=url), check=True)
finally:
    if server:
        server.terminate()
        server.wait()
